...
public class CKMetrics {
	...
	public static int getLOC(MethodBean mb){
		String content = mb.getTextContent();
	...
	public static int getMcCabeMetric(MethodBean methodBean) {
		int WMC = 0;
		WMC+=getMcCabeCycloComplexity(methodBean);
	...
	public static int getHalsteadVocabulary(MethodBean methodBean) {
		return methodBean.getTextContent().split(" ").length;
	}

	public static int getHalsteadLenght(MethodBean methodBean) {
		String tokens[] = methodBean.getTextContent().split(" ");
	...
	public static double getHalsteadVolume(MethodBean methodBean) {
		return getHalsteadLenght(methodBean) * Math.log(getHalsteadVocabulary(methodBean));
	}
...
int DIT = inizialization;
	...
	public static int getMPC(MethodBean methodBean) {
		int invocations=0;
		invocations+=methodBean.getMethodCalls().size();
	...
	public static int getIFC(MethodBean methodBean){
		int invocations=0;
		int parameters=0;
	...
	public static double getConnectivity(ClassBean cb) {
		double vertices = cb.getMethods().size();
		double edges = 0.0;
		...
		if(vertices == 1) {
			...
			connectivity = 2 * ( (edges - (vertices-1)) / ((vertices-1)) );
		} else {
	...
	public static double getCoh(ClassBean cb) {
		double numberOfMethods = cb.getMethods().size();
		double numberOfAttributes = cb.getInstanceVariables().size();
		double coh=0;

		double numberOfAttributesReferenced = 0;
		...
		if(numberOfAttributes > 0) {
			...
			coh = numberOfAttributesReferenced;
		}
	...
	public static int getTCC(ClassBean cb) {
		...
		if(totalPublicMethods == 0) {
			return 0;
		} else return (connectedPublicMethodPairs/totalPublicMethods);
	}
							...
							if(existDependencyBetween(methods.elementAt(i), methods.elementAt(j))) {
								...
								for(MethodBean callM1: methods.elementAt(i).getMethodCalls()) {
									for(MethodBean callM2: methods.elementAt(j).getMethodCalls()) {
										...
										if( (callM1.getParameters() != null) && (callM2.getParameters() != null) ) { 
											if(callM1.getName().equals(callM2.getName()) && (callM1.getParameters().size() == callM2.getParameters().size()) ) {
												connectedPublicMethodPairs++;
											}
										} else {
											if(callM1.getName().equals(callM2.getName())) {
												connectedPublicMethodPairs++;
											}
										}
	...
	// 
	public static double getICH(ClassBean cb) {
		double invocationsInClass=0.0;
		double parametersInClass=0.0;
			...
			for(MethodBean call: methodBean.getMethodCalls()) {
				...
				if(call.getBelongingClass() != null) {
					if(call.getBelongingClass().getName().equals(methodBean.getBelongingClass().getName())) {
						...
						} else parametersInClass+=0.0;
					}
				}
			}
		}

		if(parametersInClass>0.0) {
	...
	private static boolean existDependencyBetween(MethodBean m1, MethodBean m2){
		...
		for(MethodBean call: m1.getMethodCalls()) {
			if(call.getParameters() != null) {
				...
				if( (call.getName().equals(m2.getName()))) {
					return true;
				}
			}
		}
		...
		for(MethodBean call: m2.getMethodCalls()) {
			...
			if(call.getParameters() != null) {
				...
				if( (call.getName().equals(m1.getName()))) {
					return true;
				}
			}
		}

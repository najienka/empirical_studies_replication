...
out.file<- ""
file.names <- dir(path, pattern ="ngramspearman.csv") # file extension
...
for (i in 1:length(file.names)) 
  ...
  tmp.file <- read.table(file.names[i], header=TRUE,sep=",")  #open file - Not sure if you have headers or not
	...
	x = cor.test(tmp.file$references, tmp.file$semantic, method = "spearman")
   # compute correlation
...
#	pvalue=x["p.value"]
	...
	outname<-substr(file.names[i], 1, nchar(file.names[i])-17)
	
	#append the results to the same file so append = true
	#no column IDs or column names from R so both are set to false
write.table(paste(outname,x["estimate"],x["p.value"],sep=","),file=paste("corrngram.txt"), sep=",", append=TRUE , row.names=FALSE, col.names=FALSE)
